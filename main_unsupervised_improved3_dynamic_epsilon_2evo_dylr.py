...
# solution = solve([exp(-(0+y)/x)-lr_max, exp(-(length_of_phase1+y)/x)-lr_min], [x, y])
...
folder_cluster_layout = f"cluster_layout/{config['maze']}/{config['mode']}/rp{config['rp']}_ep{config['ep']}" \
                        ...
                        f"_evo2_{second_evolution}({num_saved_from_p1})_epsilon1.0--1.0--(stay at 0.1)_lr0.01" \
    ...
    for e in range(0, num_of_experiments):
        ...
        for ep in range(0, num_of_episodes):
            ...
            if ep == num_randomwalk_episodes:
                ...
                lr_at_first_evo = lr_max
                agent.epsilon = epsilon_at_first_evo
                agent.lr = lr_at_first_evo
            ...
            # #agent.lr changing
            # if num_randomwalk_episodes < ep < second_evolution:
            #     agent.lr = exp(-(ep-num_randomwalk_episodes+lr_func_b)/lr_func_a)
            # if second_evolution < ep < num_of_episodes:
            #     agent.lr = exp(-(ep-second_evolution+lr_func_b)/lr_func_a)
