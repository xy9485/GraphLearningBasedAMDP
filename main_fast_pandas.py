...
from RL_brains.RL_brain_fast_pandas import WatkinsQLambda
...
gamma = 0.99
omega = 20
epsilon = 0.5   # probability for choosing random action
    ...
    for e in range(0, num_of_experiments):
        ...
        for ep in range(0, num_of_episodes):
            ...
            if ep > (num_of_episodes / 10):
                agent.epsilon -= (0.5) / num_of_episodes   ##reduce exploration over time.


            agent.resetEligibility()        #可以修改
            # agent.resetEligibility2()

            move_count = 0
            a = agent.policy(str(env.state), env.actions(env.state))
            # path = []
            ...
            while not env.isTerminal(env.state):
                ...
                a_prime = agent.policy(str(new_state), env.actions(new_state))  ##Greedy next-action selected
                a_star = agent.policyNoRand(str(new_state), env.actions(new_state))
                ...
                if e < num_of_experiments - 1:
                    ...
                    # shaping = np.around(shaping, decimals=4)
                    # print("shaping type:",type(shaping))
                ...
                agent.learn(str(env.state), a, str(new_state), a_prime, a_star, r + shaping)  # 可以修改
                # print("q-updated")
