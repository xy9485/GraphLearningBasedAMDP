...
for rep in range(0, num_of_repetitions):
    ...
    folder_cluster_layout = f"cluster_layout/{config['maze']}/{config['mode']}/ep_{config['ep']}_maxmove_{config['max_move_count']}_length_" \
                            f"{config['min_length_to_save']}+"
        ...
        with open(fpath_paths,"w") as f:
            for repetition in path_episodes_experiments_repetitions:
                for exp in repetition:
                    for episode in exp:
                        if len(episode) > min_length_to_save_as_path:
    ...
    gensim_opt = GensimOperator(fpath_paths,fpath_embedding,fpath_cluster_layout,env)
    gensim_opt.write_embedding(size=config['representation_size'],window=config['window'])
    gensim_opt.check_unvisited_nodes()
    gensim_opt.get_cluster_layout_kmeans(package=config['package'], clusters=config['kmeans_clusters'])
    gensim_opt.write_cluster_layout()
