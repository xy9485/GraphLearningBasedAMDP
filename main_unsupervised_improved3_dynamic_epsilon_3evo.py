...
second_evolution = 400 + 3000
third_evolution = 400 + 8000
# fourth_evolution = 500 + 1500
num_saved_from_p1 = 0.8
num_saved_from_p2 = 0.8
num_of_episodes = 400 + 15000        # 可修改
...
folder_cluster_layout = f"cluster_layout/{config['maze']}/{config['mode']}/rp{config['rp']}_ep{config['ep']}" \
                        ...
                        f"_evo2_{second_evolution}({num_saved_from_p1})"\
                        f"_evo3_{third_evolution}({num_saved_from_p2})_epsilon1.0--1.0--1.0--" \
...
fig, axs = plt.subplots(num_of_repetitions, 7, figsize=(7 * 4, num_of_repetitions * 3))
    ...
    for e in range(0, num_of_experiments):
        ...
        for ep in range(0, num_of_episodes):
            ...
            if ep == num_randomwalk_episodes:
                ...
                saved_paths_phase0 = paths_period
                path_episodes.extend(saved_paths_phase0)
                paths_period = []
                ...
                axs[rep, 4].set_title(f"cluster_layout_{config['kmeans_clusters'][0]}")
                ...
                saved_paths_phase1 = sorted(paths_period, key=lambda l: len(l))[:int(num_saved_from_p1 * len(paths_period))]
                # saved_paths_period1 = paths_period
                path_episodes.extend(saved_paths_phase1)
                paths_period = []
                # get embedding from gensim and built cluster-layout
                gensim_opt.sentences = path_episodes
                ...
                for row in copy_cluster_layout:
                    for index, item in enumerate(row):
                        if row[index].isdigit():
                            ...
                            row[index] = 0
                axs[rep, 5].imshow(np.array(copy_cluster_layout), aspect='auto', cmap=plt.get_cmap("gist_ncar"))
                axs[rep, 5].set_title(f"cluster_layout_{config['kmeans_clusters'][1]}")
                # build and solve AMDP
            ...
            #Third EVO
                ...
                print("len of paths_period:", len(paths_period))
                epsilon_at_third_evo = 1
                agent.epsilon = epsilon_at_third_evo
                # saved_paths_period2 = sorted(paths_period, key=lambda l: len(l))[:num_saved_from_p1]
                saved_paths_phase2 = sorted(paths_period, key=lambda l: len(l))[:int(num_saved_from_p2 * len(paths_period))]
                # saved_paths_period2 = paths_period
                path_episodes.extend(saved_paths_phase2)
                paths_period = []
                # get embedding from gensim and built cluster-layout
                gensim_opt.sentences = path_episodes
                gensim_opt.get_clusterlayout_from_paths(size=config['representation_size'], window=config['window'],
                                                        clusters=config['kmeans_clusters'][2],
                                                        package=config['package'])
                fpath_cluster_layout = folder_cluster_layout + f"/rep{rep}_s{config['representation_size']}_w{config['window']}" \
                ...
                for row in copy_cluster_layout:
                    for index, item in enumerate(row):
                        if row[index].isdigit():
                            ...
                            row[index] = 0
                axs[rep, 6].imshow(np.array(copy_cluster_layout), aspect='auto', cmap=plt.get_cmap("gist_ncar"))
                axs[rep, 6].set_title(f"cluster_layout_{config['kmeans_clusters'][2]}")
                # build and solve AMDP
                amdp = AMDP(env=env, tiling_mode=None, dw_clt_layout=np.array(gensim_opt.cluster_layout))
                start1 = time.time()
                amdp.solveAbstraction()
            ...
            if second_evolution+(third_evolution-second_evolution)/10 <= ep <= third_evolution:
                agent.epsilon -= epsilon_at_second_evo/(third_evolution-second_evolution)

            if ep >= third_evolution+(num_of_episodes-third_evolution)/10:
                agent.epsilon -= epsilon_at_third_evo / (num_of_episodes - third_evolution)
        ...
        axs[rep, 0].axvspan(second_evolution, third_evolution, facecolor='red',alpha=0.5)
        axs[rep, 0].axis([0, num_of_episodes, 0, 3])
        axs[rep, 0].legend(loc=4)

        len_list0 = [len(x) for x in saved_paths_phase0]
        print("avg and len of random walk period0:", mean(len_list0),len(saved_paths_phase0))
        ...
        len_list1 = [len(x) for x in saved_paths_phase1]
        print("avg_length and len of period1:", mean(len_list1), len(saved_paths_phase1))
        ...
        len_list2 = [len(x) for x in saved_paths_phase2]
        print("avg_length and len of period2:", mean(len_list2), len(saved_paths_phase2))
        print("max_length, min_length and median of period2:", max(len_list2), min(len_list2), np.median(len_list2))
        axs[rep, 3].set_title(f"episodes lengths distribution of phase2")
        axs[rep, 3].hist(len_list2, bins=50, facecolor='red', density=True, alpha=0.5)
        axs[rep, 3].set_ylabel("Proportion")
        axs[rep, 3].set_xlabel("Length of episodes")
        # =====================
...
axs[0].axvspan(second_evolution,third_evolution, facecolor='red', alpha=0.5)
...
axs[1].axvspan(second_evolution, third_evolution, facecolor='red', alpha=0.5)
# axs[1].axis([0, num_of_episodes, 0, 1])
