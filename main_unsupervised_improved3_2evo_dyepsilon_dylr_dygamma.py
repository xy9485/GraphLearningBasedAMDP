...
folder_cluster_layout = f"cluster_layout/{config['maze']}/{config['mode']}/rp{config['rp']}_ep{config['ep']}" \
                        ...
                        f"_evo2_{second_evolution}({num_saved_from_p1})_eps1.0-0.1-1.0-0.1(stay at 0.1)_lr0.1-0.01_gamma0.99-0.97" \
        ...
        for ep in range(0, num_of_episodes):
            ...
            if num_randomwalk_episodes <= ep < second_evolution:
                if agent.epsilon > 0.1:
                    agent.epsilon = epsilon_max-(epsilon_max/length_of_phase1)*(ep-num_randomwalk_episodes)
                    # agent.epsilon -= epsilon_at_first_evo/(second_evolution-num_randomwalk_episodes)
            if second_evolution <= ep:
                if agent.epsilon > 0.1:
                    agent.epsilon = epsilon_max-(epsilon_max/length_of_phase2)*(ep-second_evolution)
                    # agent.epsilon -= epsilon_at_second_evo / (num_of_episodes - second_evolution)

            # #scheme2: prefer exploration a little more
            # if num_randomwalk_episodes+(second_evolution-num_randomwalk_episodes)/20 <= ep < second_evolution:
            #     agent.epsilon -= epsilon_at_first_evo/(second_evolution-num_randomwalk_episodes)
            #
            # if ep >= second_evolution+(num_of_episodes-second_evolution)/20:
            #     agent.epsilon -= epsilon_at_second_evo/(num_of_episodes-second_evolution)
            ...
            ## schme2 : linear
            if num_randomwalk_episodes <= ep < second_evolution:
                agent.gamma = gamma_max-(gamma_max/length_of_phase1)*(ep-num_randomwalk_episodes)
            if second_evolution <= ep:
                agent.gamma = gamma_max-(gamma_max/length_of_phase2)*(ep-second_evolution)
