...
abstraction_mode = [None,((5,5),(3,3))]  # 可修改
...
for i in range(0, len(plotRewards)):
    ...
    l, caps, c = axs[0].errorbar(np.arange(len(movAv)), movAv, yerr=plotErrors[i], label=labs[i], capsize=5,
...
axs[1].legend(loc=4)
axs[1].grid(True)
axs[1].set_title("reward with errorbar")
...
for i in range(0, len(mean_by_rep_move_count)):
    ...
    s = pd.Series(plotErrors[i])
    movAv = pd.Series.rolling(d, window=int(num_of_episodes / 30), center=False).mean()
    l, caps, c = axs[2].errorbar(np.arange(len(movAv)), movAv, yerr=plotErrors[i], label=labs[i], capsize=5,
                              errorevery=int(num_of_episodes / 30))
    for cap in caps:
        cap.set_marker("_")
...
axs[1].axis([0, None, None, None])
    ...
    # axs[2].plot(np.arange(len(movAv)), movAv, label=labs[i])
    # axs[2].fill_between(np.arange(len(movAv)),)
