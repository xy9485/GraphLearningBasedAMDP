...
class WatkinsQLambda():
    ...
    def check_state_exist2(self, state_str):  # in case q table is initialized as empty first
        if state_str not in self.q_table1:
            # append new state_str to q table
            to_append_q1 = state_str
            to_append_q2 = np.random.rand(4).tolist()
            to_append_e1 = state_str
            to_append_e2 = np.zeros(4).tolist()

            self.q_table1 = self.q_table1.append(to_append_q1)
            self.q_table2 = self.q_table2.append(to_append_q2)
            self.e_table1 = self.e_table1.append(to_append_e1)
            self.e_table2 = self.e_table2.append(to_append_e2)

    def check_state_exist(self, state_str):  # in case q table is initialized as empty first
        if not np.isin(state_str, self.q_table1):
            # append new state_str to q table
            to_append_q1 = state_str
    ...
    def policy(self, state, actions):
        ...
        if ran < self.epsilon * 100:
            ...
            q1_index = self.q_table1.tolist().index(str(state))
            return actions[np.argmax([self.q_table2[q1_index, a] for a in actions])]
    ...
    def learn(self, s, a, s_, a_, a_star, reward):
        ...
        q_table1_list = self.q_table1.tolist()
        e_table1_list = self.e_table1.tolist()

        q_predict = self.q_table2[q_table1_list.index(str(s)), a]
        if (s_[0], s_[1]) != self.env.goal:
            q_target = reward + self.gamma * self.q_table2[q_table1_list.index(str(s_)), a_star]
        else:
            q_target = reward

        delta = q_target - q_predict
        # print("delta:",delta,"q_max:",self.q_table.max())
        self.e_table2[e_table1_list.index(str(s)), a] = 1
