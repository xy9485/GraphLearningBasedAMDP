...
class AMDP:
    def __init__(self, env=None, tiling_mode=None, dw_clt_layout=None, gensim_opt=None, memory=None, building=None):  # tiling_mode is same with tiling_size
        if env == None:
        ...
        if building:
            ...
            self.list_of_abstract_states = np.arange(self.gensim_opt.num_clusters).tolist()      # list
            ...
            self.list_of_ground_states = self.gensim_opt.words      # list containing str
            self.cluster_labels_matching_words = self.gensim_opt.cluster_labels.tolist()  # list
            print("len(self.cluster_labels_matching_words), len(self.list_of_ground_states):", len(self.cluster_labels_matching_words), len(self.list_of_ground_states))

            self.set_transition_rewards_from_corpus()
    ...
    def get_abstract_state(self,state):
        ...
        cluster_label = self.cluster_labels_matching_words[index]
        return cluster_label

    def set_transition_rewards_from_corpus(self):
        num_abstract_states = len(self.list_of_abstract_states) + 1
    ...
    def solve_amdp(self):
        V = np.zeros(self.num_abstract_states)
        print("len(V):", len(V))
        delta = 0.2
        theta = 0.1
        ...
        while delta > theta:
            ...
            for i in range(0, len(V)):
                ...
                for a in range(len(V)):
                    ...
                    value = self.transition[a, i, a] * (self.rewards[a, i, a] + 0.99 * V[a])
                    list_of_values.append(value)
                V[i] = max(list_of_values)
    ...
    def get_value_astate(self, astate):
        value = self.V[astate]
        # print("value:",value)
        return value
