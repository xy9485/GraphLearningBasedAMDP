...
def function1(maze, big, e_mode, e_start, e_eps, subsample_factor, q_eps, repetitions, mm, rep_size, win_size, w2v, k, k_means_pkg, ):
    # from sympy.core.symbol import symbols
    # from sympy.solvers.solveset import nonlinsolve
    # from sympy import exp, solve
    print(
        f"maze:{maze}, big:{big}, e_mode:{e_mode}, e_start:{e_start}, e_eps:{e_eps}, subsample_factor:{subsample_factor}, q_eps:{q_eps}, repetitions:{repetitions}, \n"
        f"mm={mm}, rep_size={rep_size}, win_size={win_size}, w2v = {w2v}, clusters={clusters}, k_means_pkg ={k_means_pkg}")
    print("======================================================")
    ...
    # print(maze_to_plot)
    maze = ax1.pcolor(maze_to_plot, cmap=cmap, vmin=0, vmax=4, edgecolors='k', linewidth=1)
    ax1.invert_yaxis()
    ax1.axis('off')
    fig.tight_layout()
    # fig.show()
    # fig.savefig(f"./img_mazes/{env.maze_name}_big{big}.png", dpi=600, facecolor='w', edgecolor='w',
    ...
    # relative path
    folder_cluster_layout = f"./cluster_layout/{config['maze']}" \
                            f"/{config['mode']}/k{str(config['kmeans_clusters'][:1])}/rp{num_of_repetitions}_ep{num_total_episodes}_mm{max_move_count}" \
                            f"_evo1_{num_explore_episodes}_eps(1.0--0.1)_lr0.1_gamma0.999_fr10000_gr1000*flags_nr-1"
    ...
    fig, axs = plt.subplots(num_of_repetitions, 5, figsize=(5 * w, num_of_repetitions * h))
    ...
    # axs[4].axvspan(0, num_explore_episodes, facecolor='green', alpha=0.5)
    ...
    insert_row_0 = [env.maze_name, big, abstraction_mode[0], e_mode, e_start, e_eps, mm, subsample_factor, data1[1],
                   round(np.mean(longlife_exploration_std), 2), rep_size, win_size, w2v, data1[2], k_means_pkg, k, data1[3], q_eps, data1[4],
                   repetitions, "local", data1[0], round(final_reward_0,2), total_steps_0, lr, gamma, lam, omega, epsilon_e,
    ...
    insert_row_1 = [env.maze_name, big, str(abstraction_mode[1]), "None", "None", "None", "None", "None", data2[1],
                    "None", "None", "None", "None", data2[2], "None", "None", data2[3], q_eps, data2[4],
                    repetitions, "local", data2[0], round(final_reward_1,2), total_steps_1, lr, gamma, lam, omega, "None",
                    "1-0.1", config["errorbar_yerror_factor"], folder_cluster_layout]
...
if __name__ == "__main__":
    maze = 'low_connectivity2'  # low_connectivity2/external_maze21x21_1/external_maze31x31_2/strips/spiral/basic
    big = 1
    e_mode = 'RL'   # 'RL' or 'SM'
    ...
    for i in range(len(clusters)):
        function1(maze=maze, big=big, e_mode=e_mode, e_start=e_start, e_eps=e_eps, subsample_factor=subsample_factor,
                  ...
                  k=clusters[i], k_means_pkg=k_means_pkg)
